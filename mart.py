...
import os, glob
import mair
from mair.attacks import PGD, CW, DeepFool, AutoAttack, FGSM
...
from mair.defenses.advtraining.mart import MART
...
class OMART(MART):
    ...
    def __init__(self, rmodel, eps, alpha, steps, random_start=True):
        super().__init__(rmodel,eps,alpha,steps,alpha)
        #self.atk = TPGD(rmodel, eps, alpha, steps, random_start)
...
trainer = OMART(rmodel, eps=EPS, alpha=ALPHA, steps=STEPS)
